...
contract IntentOrigin is Ownable, IOriginSettler, EIP712 {
        ...
        override
    {
        ...
        emit Open(intentId, resolveFor(order, originFillerData));
    }
    ...
    function _depositAndLock(IntentData memory intent, address _caller) internal returns (bytes32 intentId) {
        ...
        intentData[intentId] = StoredIntentData(intentId, _caller, IntentStatus.OPEN);
    }

    function verifyGaslessOrder(GaslessCrossChainOrder calldata order, bytes calldata signature)
        internal
        view
        returns (bool)
    {
